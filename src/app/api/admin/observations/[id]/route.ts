...
async function resolveObservationId(
  ...
  const { data } = await supabase.from("observations").select("id").eq("slug", idOrSlug).maybeSingle();
  return data?.id ?? null;
}
...
export async function PUT(
  ...
  const { id: idOrSlug } = await params;
  const supabase = createAdminClient();
  const id = await resolveObservationId(supabase, idOrSlug);
  if (!id) return Response.json({ error: "Observation not found" }, { status: 404 });
