...
export async function GET(request: Request) {
  ...
  let query = supabase.from("release_visibility_sections").select("*");
  if (albumIdOrSlug) {
    const albumId = await resolveAlbumId(supabase, albumIdOrSlug);
    if (!albumId) return Response.json([]);
    query = query.eq("release_id", albumId);
  }
...
export async function POST(request: Request) {
  ...
  if (!body.release_id) return Response.json({ error: "album_id required" }, { status: 400 });
  if (!body.category) return Response.json({ error: "category required" }, { status: 400 });

  const { data, error } = await supabase.from("release_visibility_sections").insert({
