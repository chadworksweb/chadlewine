...
export interface ReleaseHeroItem {
  slug: string;
  title: string;
...
interface Props {
  items: ReleaseHeroItem[];
}

const TRANSITION_MS = 1100;
const TRANSITION_MS_MOBILE = 500;

export function ReleaseHero({ items }: Props) {
            ...
            >
              <div className="album-hero__stage">
                ...
                <div className="album-hero__meta">
                  <span className={`hero-lens__kind hero-lens__kind--${item.kind ?? "release"} album-hero__badge`}>{item.kind ?? "release"}</span>
