...
export function ReleaseAboutSectionEditor({ albumId }: { albumId: string }) {
  ...
  const fetchAlbum = useCallback(async () => {
    const res = await fetch(`/api/admin/releases/${encodeURIComponent(albumId)}`);
    if (!res.ok) {
      setLoading(false);
      return;
    }
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/admin/release-visibility-chat", {
        ...
        body: JSON.stringify({ release_id: album.id, geoOnly: true }),
