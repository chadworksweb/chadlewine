...
export function AlbumDataSectionEditor({ albumId, section, cat, onChanged }: Props) {
...
function hasMeaningfulPayload(slug: string, p: Record<string, unknown>): boolean {
  switch (slug) {
    ...
    case "you-might-also-like":
      return Array.isArray((p as { album_ids?: string[] }).album_ids) && ((p as { album_ids?: string[] }).album_ids?.length ?? 0) > 0;
...
function SongSliderPicker({ albumId, payload, onChange }: { albumId: string; payload: Record<string, unknown>; onChange: (p: Record<string, unknown>) => void }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/songs?album_id=${albumId}`)
      .then((r) => r.json())
...
function ArtPicker({ albumId, payload, onChange }: { albumId: string; payload: Record<string, unknown>; onChange: (p: Record<string, unknown>) => void }) {
  ...
  function setAspect(idx: number, aspect: AlbumArtAspect) {
    ...
    <div>
      ...
      {items.map((it, i) => (
        <div key={i} style={rowStyle}>
          ...
          <select value={it.aspect} onChange={(e) => setAspect(i, e.target.value as AlbumArtAspect)} className="obsv-editor__select" style={{ fontSize: "0.7rem" }}>
...
function AlbumPicker({ albumId, payload, onChange }: { albumId: string; payload: Record<string, unknown>; onChange: (p: Record<string, unknown>) => void }) {
  ...
  const picked = ((payload as { album_ids?: string[] }).album_ids) || [];
  ...
  function toggle(id: string) {
    const set = new Set(picked);
    if (set.has(id)) set.delete(id); else set.add(id);
    onChange({ album_ids: Array.from(set) });
  }

  function move(idx: number, dir: -1 | 1) {
    onChange({ album_ids: moveItem(picked, idx, dir) });
  }
