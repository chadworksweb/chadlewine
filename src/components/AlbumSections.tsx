...
export async function AlbumSections({
  albumId,
  album,
}: {
  albumId: string;
  ...
  const sections = (rawSections || []) as AlbumVisibilitySection[];
  ...
  for (const s of sections) {
    const def = getAlbumCategoryDef(s.category);
    ...
    if (s.category === "you-might-also-like" && Array.isArray((p as any).album_ids)) {
      for (const id of (p as any).album_ids) albumIdsToFetch.add(id);
    }
...
function NarrativeSection({ section, label }: { section: AlbumVisibilitySection; label: string }) {
  return (
...
function YouMightAlsoLikeSection({ payload, albumsById }: { payload: Record<string, unknown>; albumsById: Map<string, AlbumLite> }) {
  const ids = ((payload as { album_ids?: string[] }).album_ids) || [];
    ...
    <section className="album-section album-section--ymal" aria-labelledby="album-section-ymal">
      <div className="album-section__inner">
        ...
        <div className="album-section__ymal-grid">
          {items.map((a) => (
            <Link key={a.id} href={`/music/albums/${a.slug}`} className="album-section__ymal-card">
              {a.cover_art_path && (
...
// Suppress unused: ALBUM_VISIBILITY_CATEGORIES kept for future explicit ordering.
void ALBUM_VISIBILITY_CATEGORIES;
